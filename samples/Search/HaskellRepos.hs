...
repoQuery q = do
   ...
   case results of
     Left e  -> do putStrLn $ "Error: " ++ show e
                   return []
     Right r -> do putStrLn $ "Count: " ++ show n
                   return $ Github.searchReposRepos r
       where n = Github.searchReposTotalCount r

languageQuery :: Maybe GithubAuth -> String -> String  -> UTCTime -> IO (Either Github.Error Github.SearchReposResult)
languageQuery auth language field utcTime =
  let query = "q=language%3A" ++ language ++ " " ++ field ++ "%3A>" ++ date ++ "&per_page=100"
...
main = do
  ...
  results <- repoQuery q
  let repos = sortWith Github.repoPushedAt results
  forM_ repos (\r -> do
